...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    has_skills_tools = any(
        name in (agent.valid_tool_names or set())
        for name in ("skill", "skills_list", "skill_view", "skill_manage")
    )
    ...
    if not agent.skip_context_files:
        ...
        if getattr(agent, "_context_cwd_is_launch_artifact", False):
            # Preserve deterministic tool routing, but prevent a Desktop launch
            # directory from injecting Hermes's bundled contributor AGENTS.md.
            _context_cwd = None
