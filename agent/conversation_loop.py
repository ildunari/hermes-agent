    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                # Re-render prompt-cache decoration for the active provider.
                ...
                _annotation_isolated = bool(
                    getattr(agent, "_annotation_isolated", False)
                )
                if not _annotation_isolated:
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                _mixed_invalid_batch = bool(invalid_tool_calls) and any(
                    tc.function.name in _effective_tool_names
                    for tc in assistant_message.tool_calls
                )
                if _mixed_invalid_batch:
                    ...
                    _n_valid = sum(
                        ...
                        if tc.function.name in _effective_tool_names
                    )
                    agent._buffer_vprint(
                    ...
                    for tc in assistant_message.tool_calls:
                        ...
                        if _tc_name not in _effective_tool_names:
                            ...
                            content = _invalid_tool_name_error_content(
                                _tc_name, _effective_tool_names
                            )
                        else:
                ...
                for tc in assistant_message.tool_calls:
                    ...
                    try:
                        ...
                        if (
                            ...
                            and tc.function.name not in _effective_tool_names
                        ):
                ...
                if _invalid_batch_calls:
                    for tc in _invalid_batch_calls:
                        messages.append({
                            ...
                            "tool_call_id": tc.id,
                            "content": _invalid_tool_name_error_content(
                                tc.function.name, _effective_tool_names
                            ),
                        })
                    assistant_message.tool_calls = [
                        tc for tc in assistant_message.tool_calls
