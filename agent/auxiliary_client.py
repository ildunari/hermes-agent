    ...
    def create(self, **kwargs) -> Any:
        ...
        try:
            ...
            if callable(create_fn):
                ...
                # The timer can fire while responses.create() is blocked. If
                # the cancelled attempt had no stream to close at that instant,
                # close it now that it is safely attempt-owned.
                if (
                    timed_out.is_set()
                ...
                try:
                    # Some compatible hosts accept stream=True but return a
                    # completed Responses object rather than an SSE iterator.
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    """Resolve a fallback-chain API key without crossing profile scope."""
    from hermes_cli.fallback_config import codex_home_access_token, resolve_entry_api_key

    explicit = str(entry.get("api_key") or "").strip()
    if explicit:
        return explicit
    return codex_home_access_token(entry) or resolve_entry_api_key(entry)
    ...
    if (main_provider and main_model
        ...
        if runtime_base_url and main_provider == "custom":
            ...
            try:
                ...
                pass
            if _has_named_entry:
                # Keep the full custom:<name> so its configured API mode wins.
                resolved_provider = main_provider
                explicit_base_url = None
            elif runtime_base_url:
                # Config-less named custom provider: use the live endpoint.
                resolved_provider = "custom"
                explicit_base_url = runtime_base_url
                explicit_api_key = runtime_api_key or None
...
def resolve_provider_client(
    ...
    # into a different provider and override that correctly resolved model.
    #
...
def _compression_fast_lane_controls(
    ...
    *,
    ...
    if task != "compression" or max_tokens is not None:
        return max_tokens, dict(extra_body or {})
