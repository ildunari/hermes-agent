...
def _classify_write_denial(path: str) -> Optional[str]:
    """Return the denial category, or ``None`` if writes are allowed."""
    ...
    # via get_write_approval_error(). Checked after configured denies so an
    # explicit profile privacy policy remains authoritative, but before the
    # credential deny so the ``.ssh/`` prefix doesn't swallow the config file.
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "configured":
        return get_configured_denied_path_error(path, operation=verb.lower())
