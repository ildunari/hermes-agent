...
def init_agent(
    ...
    agent.pre_provider_dispatch_callback = pre_provider_dispatch_callback
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                # Capture the selected identity and a sanitized cause before a
                # successful fallback replaces agent.provider/model. The
                # fallback is universal now, but the local routing contract
                # still reports every silent init-time route change loudly.
                _sel_model = str(agent.model or "")
                _sel_provider = _explicit or "auto"
                _fb_cause = (
                    "no usable credentials resolved for primary provider "
                    f"'{_sel_provider}'"
                )
                _fb_entries = []
