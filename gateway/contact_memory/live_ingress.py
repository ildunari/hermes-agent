...
from .imessage_link_review import classify_url
from .schema import (
...
def persist_live_communication_ingress(
    *,
    ...
    if not immutable:
        return LiveCommunicationIngestResult((), 0, 0)
    if any(item.direction != "inbound" for item in immutable):
        raise ValueError("live contact ingress must be inbound")
    ...
    return LiveCommunicationIngestResult(tuple(event_ids), inserted, deduplicated)
