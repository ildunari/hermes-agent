...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    """Build platform-aware topic metadata for adapter sends.

    Most platforms route threaded sends with a generic ``thread_id`` metadata
    value. Telegram private-chat topic lanes keep the explicit private topic id
    as metadata. Live user replies route visibly with ``message_thread_id`` plus
    ``reply_to_message_id``; synthetic/resumed sends may use
...
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    # get_hermes_dir() intentionally prefers legacy image_cache if it exists,
    # but current image-generation providers save under cache/images. Allow
    # both so generated MEDIA paths remain deliverable after the recency window.
...
SUPPORTED_DOCUMENT_TYPES = {
    ...
    ".cfg": "text/plain",
    ".css": "text/css",
    # Telegram theme files are user-installable documents. They are usually
    # ZIP containers on Desktop (.tdesktop-theme) or plain/color files on
    # mobile (.attheme); route both through send_document instead of leaving
    # MEDIA tags visible in chat.
    ".tdesktop-theme": "application/octet-stream",
    ".tgios-theme": "application/octet-stream",
    ".attheme": "application/octet-stream",
    ".tgx-theme": "application/octet-stream",
...
MEDIA_DELIVERY_EXTS: Tuple[str, ...] = (
    ...
    # Web / rendered output / stylesheets
    ".html", ".htm", ".css",
    # Telegram themes (send as documents so clients can open/apply them)
    ".tdesktop-theme", ".tgios-theme", ".attheme", ".tgx-theme", ".palette", ".theme",
)
...
class MessageEvent:
    ...
    # Observed-only events are persisted as context after normal authorization,
    # but do not run the agent or produce a platform response.
...
class BasePlatformAdapter(ABC):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._pending_ephemeral_deletes: Dict[str, list[tuple[str, str]]] = {}
    ...
    def supports_draft_streaming(
        ...
        Telegram ``sendMessageDraft`` renders an animated streaming preview as
        the bot calls it repeatedly with the same ``draft_id`` and growing
        text. Adapters that implement
    ...
    def set_authorization_check(
        ...
        that are not on the configured allowlist.
        """
    ...
    def _is_sender_authorized(
        ...
        """Return whether ``user_id`` is on the allowlist, if configured."""
    ...
    async def _cleanup_pending_ephemeral_messages(self, session_key: str) -> None:
        """Delete queued ephemeral messages for a session immediately."""
        pending = self._pending_ephemeral_deletes.pop(session_key, [])
        for chat_id, message_id in pending:
            try:
                await self.delete_message(chat_id=chat_id, message_id=message_id)
            except Exception as e:
                logger.debug(
                    "[%s] Ephemeral cleanup failed for %s/%s: %s",
    ...
    def _schedule_ephemeral_delete(
        ...
        if session_key:
            self._pending_ephemeral_deletes.setdefault(session_key, []).append((chat_id, message_id))
    ...
    def _is_timeout_error(error: Optional[str]) -> bool:
        ...
        return any(
            token in lowered
            for token in ("timed out", "readtimeout", "writetimeout", "pooltimeout")
        )
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        # If this is a voice-note turn that will need auto-TTS, warm the slow
        # RTX lane immediately while STT/agent/tool work happens. Detached and
        # best-effort: Kokoro fallback still owns correctness.
    ...
    async def _send_rendered_rich_response_ordered(
        self,
        *,
        chat_id: str,
        ...
        context_badge_entry: Optional[Dict[str, Any]] = None,
        session_key: str | None = None,
        ephemeral_ttl: float = 0,
        force_document: bool = False,
    ) -> bool:
        """Send rendered rich-card/media segments in source order.

        Used only after explicit rich-card rendering so text/card/text order is
        preserved without changing the long-standing default path for ordinary
        responses. Existing MEDIA tags in that rendered response are validated
        with the same safe-root filter as normal delivery.
        """
        from gateway.rich_cards.artifacts import MediaSegment, TextSegment, response_to_ordered_segments

        segments = response_to_ordered_segments(rendered_response)
        if not segments:
            return False
            ...
            if isinstance(segment, TextSegment):
                if segment.markdown.strip():
                    result = await self._send_with_retry(
                        chat_id=chat_id,
                        content=segment.markdown.strip(),
                        reply_to=reply_to if first_segment else None,
                        metadata=notify_metadata,
                    )
                    first_segment = False
                    delivered_any = delivered_any or bool(getattr(result, "success", False))
                    if record_delivery:
                    ...
                    if (
                        ...
                        and hasattr(self, "attach_context_badge")
                    ):
                        try:
                            attach_badge = getattr(self, "attach_context_badge")
                            await attach_badge(
                                chat_id=chat_id,
                                message_id=result.message_id,
                                used=int(context_badge_entry.get("used", 0)),
                                total=int(context_badge_entry.get("total", 0)),
                                details=context_badge_entry.get("details") or {},
                                session_key=session_key,
                            )
                        except Exception as _badge_exc:
                            logger.debug("[%s] Rich-card context badge attach failed: %s", self.name, _badge_exc)
                        context_badge_entry = None
                    if (
                        ephemeral_ttl
                        and ephemeral_ttl > 0
                        and result
                        ...
                        self._schedule_ephemeral_delete(chat_id=chat_id, message_id=message_id, ttl_seconds=int(ephemeral_ttl))
                        ephemeral_ttl = 0
                continue
            if isinstance(segment, MediaSegment):
                media_path = self.validate_media_delivery_path(str(segment.path))
                if not media_path:
                    logger.warning("[%s] Skipping unsafe rich-card media path: %s", self.name, segment.path)
                    if segment.fallback_markdown:
                        result = await self._send_with_retry(
                            chat_id=chat_id,
                            content=segment.fallback_markdown,
                            reply_to=reply_to if first_segment else None,
                            metadata=notify_metadata,
                        )
                        first_segment = False
                        ...
                        if record_delivery:
                            record_delivery(result)
                    continue
                if human_delay > 0 and delivered_any:
                    await asyncio.sleep(human_delay)
                ext = Path(media_path).suffix.lower()
                media_reply_to = reply_to if first_segment else None
                try:
                    if force_document or segment.force_document:
                        result = await self.send_document(chat_id=chat_id, file_path=media_path, caption=segment.alt or None, reply_to=media_reply_to, metadata=metadata)
                    elif ext in _IMAGE_EXTS and not segment.is_voice:
                        result = await self.send_image_file(chat_id=chat_id, image_path=media_path, caption=segment.alt or None, reply_to=media_reply_to, metadata=metadata)
                    elif should_send_media_as_audio(self.platform, ext, is_voice=segment.is_voice):
                        result = await self.send_voice(chat_id=chat_id, audio_path=media_path, caption=segment.alt or None, reply_to=media_reply_to, metadata=metadata)
                    elif ext in _VIDEO_EXTS:
                        result = await self.send_video(chat_id=chat_id, video_path=media_path, caption=segment.alt or None, reply_to=media_reply_to, metadata=metadata)
                    else:
                        result = await self.send_document(chat_id=chat_id, file_path=media_path, caption=segment.alt or None, reply_to=media_reply_to, metadata=metadata)
                except Exception as media_exc:
                    logger.warning("[%s] Rich-card media delivery failed for %s: %s", self.name, media_path, media_exc)
                    result = SendResult(success=False, error=str(media_exc))
                delivered_any = delivered_any or bool(getattr(result, "success", False))
                first_segment = False
                if record_delivery:
                    record_delivery(result)
                if result and not getattr(result, "success", False) and segment.fallback_markdown:
                    fallback_result = await self._send_with_retry(
                        chat_id=chat_id,
                        content=segment.fallback_markdown,
                        reply_to=media_reply_to,
                        metadata=notify_metadata,
                    )
                    delivered_any = delivered_any or bool(getattr(fallback_result, "success", False))
                    if record_delivery:
                        record_delivery(fallback_result)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            if response:
                ...
                # Render explicit rich-card artifact blocks only after the final
                # response is complete. This avoids streaming partial YAML and
                # preserves text/card/text ordering for rendered cards.
                # Keep a pre-extract snapshot for the #29346
                # recovery/invariant below.
                _response_pre_extract = response
                try:
                    from gateway.rich_cards.artifacts import find_card_artifacts, markdown_table_auto_enabled, render_rich_cards_in_response
                    original_response = response
                    table_auto = markdown_table_auto_enabled(_platform_name(self.platform))
                    explicit_cards = bool(find_card_artifacts(response))
                    if explicit_cards or table_auto:
                        response = await asyncio.to_thread(
                            ...
                            markdown_table_auto=table_auto,
                        )
                    _has_rich_cards = explicit_cards or (table_auto and "MEDIA:" in response and response != original_response)
                    if _has_rich_cards:
                        _rich_reply_anchor = _reply_anchor_for_event(event)
                        _rich_metadata = dict(_thread_metadata or {})
                        _rich_metadata["notify"] = True
                        _rich_badge_entry = None
                        _pending_badges = getattr(self, "_pending_context_badges", None)
                        if _pending_badges:
                            _rich_badge_entry = _pending_badges.get(str(event.source.chat_id))
                        _rich_delivered = await self._send_rendered_rich_response_ordered(
                            chat_id=event.source.chat_id,
                            rendered_response=response,
                            metadata=_rich_metadata,
                            reply_to=_rich_reply_anchor,
                            record_delivery=_record_delivery,
                            context_badge_entry=_rich_badge_entry,
                            session_key=session_key,
                            ephemeral_ttl=_ephemeral_ttl or 0,
                            force_document=force_document_attachments,
                        )
                        if _rich_delivered:
                            if _pending_badges and _rich_badge_entry:
                                _pending_badges.pop(str(event.source.chat_id), None)
                            response = None
                        else:
                            # Renderer/delivery fallback: continue through the
                            # legacy text path with the preserved fallback text.
                            response = MEDIA_TAG_CLEANUP_RE.sub("", response).strip()
                except Exception as _rich_exc:
                    logger.warning("[%s] Rich-card pre-pass failed; using text fallback: %s", self.name, _rich_exc)
                ...
                if response is not None and not (text_content or images or local_files or media_files):
                ...
                if text_content and not _tts_caption_delivered:
                    ...
                    try:
                        ...
                        if (
                            ...
                            _badge_entry = _pending_badges.pop(str(event.source.chat_id), None)
                            if _badge_entry:
                                await self.attach_context_badge(
                                    chat_id=event.source.chat_id,
                                    message_id=result.message_id,
                                    used=int(_badge_entry.get("used", 0)),
                                    total=int(_badge_entry.get("total", 0)),
                                    details=_badge_entry.get("details") or {},
                                    session_key=_badge_entry.get("session_key"),
                                )
                    except Exception as _badge_exc:
                        logger.debug("[%s] Context badge post-send attach failed: %s", self.name, _badge_exc)
