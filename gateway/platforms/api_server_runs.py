...
def _initialize_run_state(self, *, store_factory) -> None:
    ...
    # Compact delegated-child lifecycle records retained per run.
...
def _set_run_status(
    ...
    **fields: Any,
    ...
    if run_id in getattr(self, "_run_idempotency_ids", set()) and should_persist:
        try:
...
async def _handle_runs(
    ...
    *,
    ...
    # ``X-Hermes-Session-Key``. Falling straight through to ``run_id`` made
    # the run id the conversation identity and re-keyed every affinity
    # surface once per request (#96811). Explicit/chained sessions must not
    # be rebound to this request's header key.
    ...
    async def _run_and_close():
        try:
            ...
            # Delegated children read these immutable ownership markers when
            # constructing their compact lifecycle records.
            ...
            def _run_sync():
                ...
                with self._profile_scope(request_profile):
                    try:
                        ...
                        # /v1/runs owns its agent lifecycle, so it records the
                        # declared conversation itself rather than through
                        # _run_agent. An explicit body/chained session turns
                        # this precedence gate off.
