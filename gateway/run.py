...
def _cron_tick_profile_homes(config: object) -> list[tuple[str, "Path"]]:
    """Return the served homes plus the process-active profile for the host ticker."""
...
def _start_gateway_start_cron_and_housekeeping(runner):
    ...
    if isinstance(cron_provider, InProcessCronScheduler) and cron_profile_homes:
        ...
        cron_start_kwargs["profile_gate"] = lambda name, home: (
            _cron_profile_gate(name, home) and _gateway_cron_profile_gate(name, home))
