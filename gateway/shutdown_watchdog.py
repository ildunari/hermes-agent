...
async def loop_heartbeat_forever(
    ...
    *initiated* by the loop, so a frozen loop still lets the file age.
    ...
    return value merged into the heartbeat payload (for example, the live
    ``active_agents`` snapshot). A failing provider is swallowed so it cannot
    ...
    async def _write_off_loop() -> None:
        # Both disk writes are off-loop: either can end in an fsync and must not
        # freeze the event loop that this heartbeat is intended to monitor.
        try:
            heartbeat_extra = dict(
                _collect_heartbeat_extra(extra_provider) or {}
            )
            # The locally sampled payload must not override the authoritative
            # witness state.
            heartbeat_extra["loop_tick_socket"] = tick_server is not None
