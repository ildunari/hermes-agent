  ...
  const filteredModels = useMemo(
    () =>
      fuzzyRank(
        ...
        positions: selectedProvider?.model_labels?.[r.item]
          ? []
          : r.positions.filter((i) => i >= 0 && i < r.item.length),
      })),
    [models, selectedProvider, trimmedQuery, queryMatchesSelectedProviderOnly],
  );
