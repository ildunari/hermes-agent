    ...
    *,
    ...
    def _callback(
        ...
        if event_type == "subagent.complete":
            ...
            record = (
                _compact_record(raw_status, usage=usage, error=error, terminal=True)
    ...
    **_kwargs,
    ...
    try:
        ...
        if interrupted:
            ...
            # A structured failure must win over the summary-presence
            # heuristic: fatal results can carry diagnostic final_response.
            status = "failed"
        elif _schema_valid is False:
            # A response that still violates the declared output schema after
            # the bounded retry is unusable and must not report completed.
            status = "failed"
        ...
        if interrupted:
            ...
            exit_reason = str(result.get("turn_exit_reason") or "error")
        elif completed:
...
def delegate_task(
    ...
    # Background (async) delegation is opt-in. By default, top-level model
    # calls run synchronously: a batch still fans out children in parallel, but
    # this tool joins them and returns one consolidated result to the parent.
    # With background=true, the whole fan-out runs as one detached async unit
    # and re-enters later with one consolidated result block.
    ...
    effective_max_iter = hard_max_iter
    ...
    # unexpanded multi-word template markers, 1-task batches) before routing
    # work, credential resolution, or child construction. The single-`goal`
    # form is deliberately exempt; duplicate goals are allowed (best-of-N).
    if tasks is not None and isinstance(tasks, list):
        batch_error = _validate_batch_tasks(task_list)
        if batch_error:
            return tool_error(batch_error)

    # Coerce and validate optional per-task output schemas before routing or
    # credential resolution so malformed contracts fail without side effects.
    ...
    _header_creds = task_creds[0] if task_creds else {}
...
def _build_top_level_description() -> str:
    ...
    return (
        "Spawn isolated subagents; each has its own conversation, terminal, and "
        "toolset, and only its final summary returns. Pass every task in `tasks` — "
        "one entry spawns one subagent, several run in parallel. Limits and "
        "budgets are described in the parameters.\n\n"
        "Default execution is SYNCHRONOUS: delegate_task returns after children "
        "finish, consolidating batches. Set background=true only for detached "
        "work whose result will re-enter later; do NOT wait or poll on it.\n\n"
        "LIVE ORCHESTRATION: action='list' inspects children, action='steer' "
        "redirects one by subagent_id + message, and action='stop' ends one early.\n\n"
        ...
        "process exit discards running background subagents.\n\n"
        "RULES:\n"
        ...
        + restrictions_rule
        + "- Children inherit the parent model/fallbacks unless overridden per "
        "task or pinned via delegation.provider / delegation.model. "
        "enabled_toolsets is narrowing-only; omit 'profile' to inherit. "
...
def _build_tasks_param_description() -> str:
    ...
    return (
        ...
        "is a one-entry array. Per-task overrides beat top-level defaults."
    )
...
def _build_dynamic_schema_overrides() -> dict:
    ...
    overrides_params["properties"]["budget_class"]["description"] = (
        _build_budget_param_description()
    )
    overrides_params["properties"]["tasks"]["items"]["properties"][
        "budget_class"
    ]["description"] = _build_budget_param_description(per_task=True)
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "provider": {
                "type": "string",
                "description": (
                    "Optional configured provider override. Credentials are "
                    "resolved by Hermes; API keys are not accepted."
                ),
            },
            "reasoning_effort": {
            ...
            "enabled_toolsets": {
                ...
                "items": {"type": "string"},
                "description": (
                    "Optional narrowing-only toolset allowlist; cannot grant "
                    "capabilities absent from the parent or bypass delegate blocks."
                ),
            },
            "profile": {
                "type": "string",
                "description": (
                    "Optional requested profile. Currently returns a structured "
                    "refusal; omit to inherit the parent profile."
                ),
            },
            "budget_class": {
                "type": "string",
            ...
            "tasks": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "provider": {
                            "type": "string",
                            "description": "Per-task configured provider override.",
                        },
                        "reasoning_effort": {
                            "oneOf": [{"type": "string"}, {"type": "boolean"}],
                        ...
                        "profile": {
                            "type": "string",
                            "description": (
                                "Requested profile. Currently returns a structured "
                                "refusal; omit to inherit parent."
                            ),
                        },
                        "budget_class": {
                            "type": "string",
            ...
            "background": {
                "type": "boolean",
                "description": (
                    "Optional. Defaults to false. When false, delegation is "
                    "synchronous: tasks run in parallel and return one consolidated "
                    "results array after all children finish. When true, the whole "
                    "delegation runs as a detached, process-local background unit "
                    "and re-enters later."
                ),
            },
            "action": {
...
def _model_background_value(args: dict, parent_agent=None) -> bool:
    ...
    Delegations from the top-level agent default to synchronous execution; a
    model can opt into detached execution with ``background=true``. Delegations
    from an orchestrator subagent (depth > 0) are always synchronous because
    the child needs its workers' results within its own turn. The live path is
    ``run_agent._dispatch_delegate_task``; this lambda mirrors it for the rare
    case the intercept is bypassed.
    """
