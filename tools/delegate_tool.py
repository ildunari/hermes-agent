...
def _build_top_level_description() -> str:
    ...
    return (
        ...
        "(limits, nesting, and iteration budgets are in the parameter "
        "descriptions).\n\n"
        "Default execution is SYNCHRONOUS: delegate_task returns when the "
        "subagent(s) finish, and a batch returns one consolidated results "
        "array after all children finish. Set background=true only for "
        "detached fire-and-forget work — dispatch then returns immediately "
        "with live transcript paths and the completed result re-enters the "
        "conversation on its own; do NOT wait or poll on it.\n\n"
