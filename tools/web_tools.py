...
_DEFAULT_ASYNC_IS_SAFE_URL = async_is_safe_url
...
async def _safe_url_allowed(url: str) -> bool:
    """URL safety hook supporting both async and legacy sync monkeypatches."""
    if async_is_safe_url is not _DEFAULT_ASYNC_IS_SAFE_URL:
        result = async_is_safe_url(url)
    else:
...
async def web_extract_tool(
    ...
    links) require Firecrawl; other backends support basic markdown/html.
    Configured extract backends include Firecrawl, Tavily, Exa, Parallel, and Keenable.
    Pages over ``char_limit`` are head+tail truncated with an explicit footer;
    the full text is stored under cache/web and the footer tells the model how
    to read_file the omitted middle. Inline base64 images are replaced with
...
def check_web_api_key() -> bool:
    ...
    if configured:
        if _is_backend_available(configured):
            return True
        # Built-in selections are strict: a broken selected backend must not
        # be masked by another configured provider. Custom plugin selections
        # need the registry readiness probe below because their active provider
        # can be available even when the cheap built-in probe has no entry.
