...
// Rows are memoized because DOM-budget and scroll-state updates rebuild the
// visible slice frequently. Stable group identity lets settled transcript rows
// bail out instead of re-rendering markdown, code cards, and tool blocks.
const TurnRow = memo(function TurnRow({ components, group, resetKey, virtualized }: TurnRowProps) {
  return (
    <div className={messageGroupClassName(group, virtualized)} data-slot="aui_message-group">
        ...
        {group.kind === 'turn' ? (
          ...
          >
            ...
            {group.indices.length > 1 && (
              <div className={turnTailClassName(virtualized)} data-slot="aui_turn-tail">
