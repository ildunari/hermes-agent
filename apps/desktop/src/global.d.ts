...
declare global {
  interface BrowserUploadConsentFile {
    displayName: string
    mimeType: string
  ...
  interface BrowserUploadConsentDetail {
    accept: string
  ...
  interface BrowserConsentPrompt {
    ...
    navigationPolicy?: {
      ...
      provenance: readonly Record<string, boolean | number | string>[]
      pslVersion: string
  ...
  interface BrowserActivityPersistedRow {
    bindingGeneration: number
    category: 'action' | 'hand-back' | 'navigate' | 'pause' | 'snapshot' | 'stop' | 'takeover'
    certainty: 'completed' | 'failed' | 'not_started' | 'outcome_unknown' | null
    ...
    sessionLineageId: string | null
    siteIdentity: string | null
    source: 'agent' | 'consent' | 'human' | 'policy' | 'system' | 'transport'
    tabIncarnationId: string
    terminalAt: number | null
    workspaceId: string
  }

  interface BrowserRestoreDescriptor {
    createdAt: number
    ordinal: number
    pinned: boolean
    restoreId: string
  ...
  interface BrowserVisitPersistedRow {
    origin: string
  ...
  interface BrowserOriginPersistedRow {
    lastUsedAt: number
    origin: string
  }

  interface BrowserPermissionPersistedRow {
    decidedAt: number
    decision: 'allow' | 'deny'
    origin: string
    permission: string
  ...
  interface BrowserGuestViewportReport {
    devicePixelRatio: number
    height: number
    width: number
  }

  interface BrowserGuestReportRequest {
    documentGeneration?: number
    generation: string
    kind: 'viewport'
    tabId: string
  ...
  type BrowserGuestReportResponse =
    | {
        documentGeneration: number
        error?: undefined
        ok: true
        value: BrowserGuestViewportReport
      }
    ...
    hermesDesktop: {
      ...
      browserGuest: {
        prepare: (request: {
          ...
          tabId: string
          workspaceId: string
        }) => Promise<{ attachmentUrl?: string; error?: string; generation?: string; ok: boolean }>
        activate: (request: {
          generation: string
          tabId: string
          url: string
        }) => Promise<{ error?: string; ok: boolean }>
        bindAutomation: (request: {
          guestGeneration: string
          requireFreshSnapshot?: boolean
          tabId: string
          taskGeneration: number
          taskId: string
        }) => Promise<{ error?: string; ok: boolean; role?: 'automation' }>
        mintResource: (request: {
          generation: string
          ...
          error?: string
          guestUrl?: string
          kind?: 'remote-artifact' | 'remote-preview'
          localRef?: string
          ok: boolean
        }>
        onFreshSnapshot: (callback: (event: {
          guestGeneration: string
          surfaceEpoch: string
          tabId: string
          taskGeneration: number
        ...
        onConsentRequested: (callback: (prompt: BrowserConsentPrompt) => void) => () => void
        onConsentResolved: (callback: (outcome: {
          consentId: string
          reason: 'allow' | 'deny' | 'expired' | 'ordinary-for-task' | 'revoked' | 'stale'
        }) => void) => () => void
        resolveConsent: (response: {
          consentId: string
          decision: 'allow' | 'deny' | 'ordinary-for-task'
        }) => Promise<{ error?: string; ok: boolean }>
        release: (request: { generation: string; tabId: string }) => Promise<{ ok: boolean }>
        report: (request: BrowserGuestReportRequest) => Promise<BrowserGuestReportResponse>
        resolveAnnotations: (request: {
          generation: string
          records: unknown
          tabId: string
          workspaceId: string
          ...
          projections?: Array<{
            ...
            health: 'ambiguous' | 'resolved' | 'shifted' | 'stale' | 'unsupported'
          }>
        }>
        exportAnnotationScreenshot: (request: {
          ...
          tabId: string
          workspaceId: string
        }) => Promise<{ canceled?: boolean; error?: string; ok: boolean }>
        onRetired: (callback: (event: {
          guestGeneration: string
          reason:
            | 'activation-failed'
            | 'crashed'
      ...
      browserActivity: {
        append: (request: {
          event: {
            bindingGeneration: number
            category: BrowserActivityPersistedRow['category']
            certainty?: BrowserActivityPersistedRow['certainty']
            checkpointAttempted?: boolean
            checkpointAvailableAtCapture?: boolean
            operationId?: string | null
            phase: BrowserActivityPersistedRow['phase']
            private?: boolean
            reasonCode?: string | null
            sessionId?: string | null
            sessionLineageId?: string | null
            siteIdentity?: string | null
            source: BrowserActivityPersistedRow['source']
            tabIncarnationId: string
            workspaceId: string
          }
          profile: string
        }) => Promise<{ error?: 'degraded' | 'invalid'; ok: boolean; row?: BrowserActivityPersistedRow }>
        clear: (request: { profile: string; workspaceId?: string }) => Promise<{ ok: boolean }>
        deleteSession: (request: { profile: string; sessionId: string }) => Promise<{ ok: boolean }>
        list: (request: { limit?: number; profile: string; workspaceId?: string }) => Promise<{
          degraded: boolean
          rows: BrowserActivityPersistedRow[]
        }>
      }
      browserState: {
        clearBrowsingData: (request: { profile: string }) => Promise<{
          metadata: boolean
          ok: boolean
          permissions: boolean
          siteData: boolean
        ...
        clearSiteData: (request: { origin?: string; profile: string }) => Promise<{
          ok: boolean
          permissions: boolean
          siteData: boolean
        }>
        exportQuarantinedMetadata: (request: { profile: string }) => Promise<{
          canceled?: boolean
          ok: boolean
        }>
        history: (request: { limit?: number; profile: string }) => Promise<{
          degraded: boolean
          rows: BrowserVisitPersistedRow[]
        }>
        origins: (request: { profile: string }) => Promise<{
          degraded: boolean
          rows: BrowserOriginPersistedRow[]
        }>
        permissions: (request: { limit?: number; profile: string }) => Promise<{
          degraded: boolean
          rows: BrowserPermissionPersistedRow[]
        }>
        remove: (request: { epoch: string; profile: string; restoreId: string }) => Promise<{ ok: boolean }>
        removePermission: (request: {
          origin: string
          permission: string
          profile: string
        }) => Promise<{ ok: boolean }>
        resetWorkspace: (request: {
        ...
        repair: (request: { mode: 'reset-metadata' | 'retry'; profile: string }) => Promise<{
          activity: boolean
          epoch: string
          metadata: boolean
          ok: boolean
        ...
        setRestoreEnabled: (request: { enabled: boolean; profile: string }) => Promise<{ ok: boolean }>
        select: (request: {
          epoch: string
          profile: string
          restoreId: null | string
          workspaceId: string
        }) => Promise<{ ok: boolean }>
        snapshot: (request: { profile: string }) => Promise<{
          degraded: boolean
        ...
        upsert: (request: {
          descriptor: BrowserRestoreDescriptor
          epoch: string
          profile: string
        }) => Promise<{ ok: boolean; persisted?: BrowserRestoreDescriptor }>
      }
