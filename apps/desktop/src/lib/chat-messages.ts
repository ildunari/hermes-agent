...
export type GatewayEventPayload = {
  ...
  session_id?: string
  title?: string
  // session.info — raw backend routing requires schema validation before use.
  runtime_routing?: unknown
  // The stored (durable) session id for this runtime session lets Desktop map
  // runtime→stored for background sessions it hasn't opened.
