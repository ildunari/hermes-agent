...
const SIDEBAR_EXCLUDED_SOURCES = ['cron', 'kanban', 'subagent', 'tool', 'smoke-test', ...MESSAGING_SESSION_SOURCE_IDS]
  ...
  const refreshSessions = useCallback(async () => {
    ...
    try {
      ...
      if (
        ...
        const { recents, cron, messaging } = result
        ...
        // platform in the UI.
        const messagingRows = dropTombstoned(messaging.sessions.filter(session => isMessagingSource(session.source)))
