...
export function useSubmitPrompt(deps: SubmitPromptDeps) {
  ...
  return useCallback(
    async (rawText: string, options?: SubmitTextOptions) => {
      ...
      // React Router commits navigation on the next render. During a first-send
      // create, both the pre-create route and the newly created route are valid
      // observations while attachment sync awaits, so drift tracks a set.
      const allowedRouteTokens = new Set([getRouteToken()])

      const sessionContextDrifted = (): boolean =>
        targetStartedInCurrentView &&
        (selectedStoredSessionIdRef.current !== startingStoredSessionId || !allowedRouteTokens.has(getRouteToken()))
