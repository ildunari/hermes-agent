...
contextBridge.exposeInMainWorld('hermesDesktop', {
  ...
  browserGuest: {
    prepare: request => ipcRenderer.invoke('hermes:browser-guest:prepare', request),
    activate: request => ipcRenderer.invoke('hermes:browser-guest:activate', request),
    bindAutomation: request => ipcRenderer.invoke('hermes:browser-guest:bind-automation', request),
    mintResource: request => ipcRenderer.invoke('hermes:browser-resource:mint', request),
    release: request => ipcRenderer.invoke('hermes:browser-guest:release', request),
    report: request => ipcRenderer.invoke('hermes:browser-guest:report', request),
    resolveAnnotations: request => ipcRenderer.invoke('hermes:browser-guest:resolve-annotations', request),
    exportAnnotationScreenshot: request =>
      ipcRenderer.invoke('hermes:browser-guest:export-annotation-screenshot', request),
    onRetired: callback => {
      const listener = (_event, payload) => callback(payload)
      ipcRenderer.on('hermes:browser-guest:retired', listener)

      return () => ipcRenderer.removeListener('hermes:browser-guest:retired', listener)
    },
    onFreshSnapshot: callback => {
      const listener = (_event, payload) => callback(payload)
      ipcRenderer.on('hermes:browser-guest:fresh-snapshot', listener)

      return () => ipcRenderer.removeListener('hermes:browser-guest:fresh-snapshot', listener)
    },
    onConsentRequested: callback => {
      const listener = (_event, prompt) => callback(prompt)
      ipcRenderer.on('hermes:browser-consent:requested', listener)

      return () => ipcRenderer.removeListener('hermes:browser-consent:requested', listener)
    },
    onConsentResolved: callback => {
      const listener = (_event, outcome) => callback(outcome)
      ipcRenderer.on('hermes:browser-consent:resolved', listener)

      return () => ipcRenderer.removeListener('hermes:browser-consent:resolved', listener)
    },
    resolveConsent: response => ipcRenderer.invoke('hermes:browser-consent:resolve', response),
    unbindAutomation: request => ipcRenderer.invoke('hermes:browser-guest:unbind-automation', request),
    revokeLocal: request => ipcRenderer.invoke('hermes:browser-guest:revoke-local', request),
    stopAndClose: request => ipcRenderer.invoke('hermes:browser-guest:stop-and-close', request)
  },
  browserActivity: {
    append: request => ipcRenderer.invoke('hermes:browser-activity:append', request),
    clear: request => ipcRenderer.invoke('hermes:browser-activity:clear', request),
    deleteSession: request => ipcRenderer.invoke('hermes:browser-activity:delete-session', request),
    list: request => ipcRenderer.invoke('hermes:browser-activity:list', request)
  },
  browserState: {
    clearBrowsingData: request => ipcRenderer.invoke('hermes:browser-state:clear-browsing-data', request),
    clearMetadata: request => ipcRenderer.invoke('hermes:browser-state:clear-metadata', request),
    clearSiteData: request => ipcRenderer.invoke('hermes:browser-state:clear-site-data', request),
    exportQuarantinedMetadata: request =>
      ipcRenderer.invoke('hermes:browser-state:export-quarantined-metadata', request),
    history: request => ipcRenderer.invoke('hermes:browser-state:history', request),
    origins: request => ipcRenderer.invoke('hermes:browser-state:origins', request),
    permissions: request => ipcRenderer.invoke('hermes:browser-state:permissions', request),
    remove: request => ipcRenderer.invoke('hermes:browser-state:remove', request),
    removePermission: request => ipcRenderer.invoke('hermes:browser-state:remove-permission', request),
    resetWorkspace: request => ipcRenderer.invoke('hermes:browser-state:reset-workspace', request),
    repair: request => ipcRenderer.invoke('hermes:browser-state:repair', request),
    setPermission: request => ipcRenderer.invoke('hermes:browser-state:set-permission', request),
    setRestoreEnabled: request => ipcRenderer.invoke('hermes:browser-state:set-restore-enabled', request),
    select: request => ipcRenderer.invoke('hermes:browser-state:select', request),
    snapshot: request => ipcRenderer.invoke('hermes:browser-state:snapshot', request),
    upsert: request => ipcRenderer.invoke('hermes:browser-state:upsert', request)
  },
  openSessionWindow: (sessionId, opts) => ipcRenderer.invoke('hermes:window:openSession', sessionId, opts),
  openNewSessionWindow: (profile?: string) => ipcRenderer.invoke('hermes:window:openNewSession', profile),
