...
function venvHermesShimPath(updateRoot) {
  ...
  return IS_WINDOWS ? path.join(venvRoot, 'Scripts', 'hermes.exe') : path.join(venvRoot, 'bin', 'hermes')
}
...
function writeActiveDesktopProfile(name, { requireLocal = true } = {}) {
...
ipcMain.handle('hermes:profile:remember', async (_event, name) => ({
  profile: writeActiveDesktopProfile(name, { requireLocal: false })
}))
