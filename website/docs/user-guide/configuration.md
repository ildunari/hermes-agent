...
dashboard:
  ...
  ws_orphan_reap_grace_s: 20.0 # Idle cleanup / running-turn recheck interval (seconds)
...
- `ws_orphan_reap_grace_s` — how often Hermes checks a WS-detached session. Idle sessions are collected after the interval; active turns and delegations continue detached and are rechecked until they settle. A disconnect never interrupts an active turn. (`HERMES_TUI_WS_ORPHAN_REAP_GRACE_S` remains as an internal override.)
