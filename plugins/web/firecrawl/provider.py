...
class FirecrawlWebSearchProvider(WebSearchProvider):
    ...
    async def extract(self, urls: List[str], **kwargs: Any) -> List[Dict[str, Any]]:
        ...
        # The anonymous ring implements basic page extraction only. Advanced
        # Firecrawl modes require the selected SDK/client so their prompt,
        # schema, and scrape options are not silently discarded.
        if mode in {"markdown", "html"} and _use_keyless_ring():
            # No credentials and no managed gateway: ring dispatch with
            # next-in-line failover on rate limits (default-on free tier).
