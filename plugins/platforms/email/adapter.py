...
def check_email_requirements() -> bool:
    ...
    pwd = bool(
        _get_secret("EMAIL_PASSWORD", "")
        or _get_secret("EMAIL_PASSWORD_CMD", "").strip()
        or email_password_configured(getenv=_get_secret)
    )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._password = resolve_email_password(getenv=_get_secret)
    ...
    def _parse_fetched_message(self, uid: bytes, raw_email: "bytes | bytearray") -> Optional[Dict[str, Any]]:
        ...
        allowed = allowed_inbound_email_senders(getenv=_get_secret)
        if allowed and sender_addr.lower() not in allowed:
            logger.debug(
                "[Email] Dropping non-allowlisted sender before decode/cache: %s",
                sender_addr,
            )
            return None
    ...
    async def _dispatch_message(self, msg_data: Dict[str, Any]) -> None:
        ...
        allowed = {
            address.strip().lower()
            for address in _get_secret("EMAIL_ALLOWED_USERS", "").split(",")
            if address.strip()
        }
        if not allowed:
    ...
    def _send_email(
        ...
        if not email_recipient_allowed(to_addr, getenv=_get_secret):
...
async def _standalone_send(
    ...
    *,
    ...
    password = resolve_email_password(getenv=_get_secret)
    ...
    if not email_recipient_allowed(chat_id, getenv=_get_secret):
