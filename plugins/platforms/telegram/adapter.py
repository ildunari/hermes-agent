...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def _thread_kwargs_for_send(
        ...
        """
        if cls._metadata_has_explicit_direct_topic_only(metadata):
            topic_id = cls._metadata_direct_messages_topic_id(metadata)
            return {
                "message_thread_id": None,
                "direct_messages_topic_id": int(topic_id) if topic_id is not None else None,
            }
        if metadata and metadata.get("telegram_dm_topic_reply_fallback"):
            # Telegram private-topic lanes created by Hermes must keep using
            # message_thread_id. Falling back to direct_messages_topic_id makes
            # final replies land in All/vanish from the visible topic on Kosta's
            # Telegram client path.
    ...
    def _rich_eligible(self, content: str, metadata: Optional[Dict[str, Any]] = None) -> bool:
        ...
        return bool(
            ...
            and self._rich_delivery_enabled(content)
