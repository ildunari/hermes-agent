...
def acquire_lease(root: Path, run_id: str):
    ...
    try:
        ...
        handle.close()
        raise RuntimeError("update lease is held by another worker")
    handle.seek(0)
    raw = handle.read()
    if raw:
        try:
        ...
        if pid > 0 and pid != os.getpid() and ps_start_time(pid) == expected_start:
            handle.close()
            raise RuntimeError(f"lease held by live process {pid}")
    payload = {
        "run_id": run_id,
        "pid": os.getpid(),
...
def worker_command(
    ...
    return owned_command(
        ...
        origin_limit,
        env,
        run_dir(root, run_id) / "abort.request",
        allow_failure,
    )
...
def deploy(
    ...
    def stage_action() -> None:
        ...
        worker_command(root, run_id, ["ssh", "macbook", remote], repo, "macbook-stage", 600)
    ...
    receipted(
        ...
        stage_probe,
        stage_action,
        lambda: ensure_not_aborted(root, run_id),
    )
