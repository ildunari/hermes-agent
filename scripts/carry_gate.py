...
def worktree(root: Path, commit: str) -> tuple[Path, tempfile.TemporaryDirectory[str]]:
    holder = tempfile.TemporaryDirectory(prefix="hermes-carry-gate-")
    target = Path(holder.name) / "tree"
    run(root, "git", "worktree", "add", "--detach", str(target), commit)
    return target, holder


def cleanup(root: Path, target: Path, holder: tempfile.TemporaryDirectory[str]) -> None:
...
def staged(args: argparse.Namespace) -> int:
    ...
    target, holder = worktree(root, "HEAD")
    try:
...
def pushed(args: argparse.Namespace) -> int:
    ...
    for fields in lines:
        ...
        if local_sha == ZERO_SHA:
            continue
        target, holder = worktree(root, local_sha)
        try:
