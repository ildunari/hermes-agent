...
def try_register_running_job(job_id: str, probe_binding: dict | None = None) -> bool:
    ...
    ``probe_binding`` is copied at dispatch so a shutdown-time completion
    cannot accidentally acknowledge a replacement probe configuration.
    ...
    with _running_lock:
        ...
        _running_probe_snapshots[job_id] = (
            dict(probe_binding) if isinstance(probe_binding, dict) else None
        )
...
def mark_running_jobs_interrupted(
    ...
    *,
    ...
    with _running_lock:
        probe_snapshots = {
            job_id: _running_probe_snapshots.get(job_id)
...
def _run_job_script(
    ...
    * anything else — run with the current Python interpreter
    ...
    try:
        ...
        # build_subprocess_env preserves ambient HERMES_HOME (and may bridge a
        # context override), but the scheduler's resolved profile is
    ...
    *,
    ...
    try:
        ...
        agent = AIAgent(
            ...
            # Cron has no human-in-the-loop need for skill/memory review forks
            # (~30K tokens/event).
    ...
    *,
    ...
    try:
        ...
        if (
            ...
            ack_metadata = dict(probe_binding)
        mark_kwargs = {
    ...
    try:
        ...
        def _submit_with_guard(job: dict, pool: concurrent.futures.ThreadPoolExecutor):
            ...
            if _interpreter_shutting_down():
                ...
                return None
            if not try_register_running_job(job_id, job.get("probe_binding")):
