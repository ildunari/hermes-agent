...
def _finish_completed_run(d: _RunDelivery, fire_owner: Optional[str], execution_id: str) -> bool:
    ...
    if snapshot is not None:
        ...
        nonce = str(snapshot.get("nonce") or "")
        ...
        if (
            ...
            and d.success
            and d.delivery_error is None
            and d.deliver_content.strip()
            == f"HERMES_PROACTIVE_ALARM_PROBE_ACK_REQUEST {nonce} {generation}"
            and job.get("deliver") == snapshot.get("target")
            and job.get("script") == snapshot.get("script")
        ):
...
def _run_one_job_body(
    ...
    try:
        ...
        d = _RunDelivery(
            job=job, success=success, error=error, probe_run_snapshot=probe_run_snapshot)
        try:
