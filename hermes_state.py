...
class SessionDB:
            ...
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    if attempt < self._WRITE_MAX_RETRIES - 1:
                        ...
                        time.sleep(jitter)
                        continue
                # OperationalError subclasses DatabaseError, and several
                # SQLite builds classify FTS5 corrupt-structure failures here.
                # Try the same one-shot rebuild before propagating a non-lock
                # operational error; otherwise the broader handler below is
                # unreachable for the most common FTS failure class.
    ...
    def optimize_fts_storage(
        ...
        *,
        ...
        if legacy and not pending:
            self._demote_legacy_fts_to_trash()
            # The compact v23 trigram index is always rebuilt by the explicit
            # optimizer, even when the legacy inline index was disabled.
            with self._lock:
                self._trigram_available = (
                    self._fts_table_probe(
                        self._conn, "messages_fts_trigram"
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 21:
                # v21: per-model usage attribution (issue #51607). Going
                # v20 was already used by the local last_active migration, so
                # this migration must advance independently for existing DBs.
    ...
    def get_messages_as_conversation(
        ...
        with self._lock:
            ...
            select_cols = (
                ...
                "observed, timestamp, api_content, display_kind, display_metadata"
            )
