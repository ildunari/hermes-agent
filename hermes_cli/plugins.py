...
class PluginContext:
    ...
    # -- gateway conversation extension registration ------------------------

    @_serialized_replacement
    def register_gateway_conversation_extension(
        self,
        extension,
    ) -> Optional[PluginRegistration]:
        """Register one atomic gateway conversation-extension bundle.

        The bundle is a :class:`gateway.conversation_extensions.GatewayConversationExtension`
        containing every subinterface the plugin implements — admission/route
        policy, turn augmentation, final-dispatch tool authorization,
        ingress/post-turn observation, and lifecycle. Registering the whole
        bundle at once means a reload can never leave one generation's routing
        policy live beside another generation's tool policy.

        Registration is profile-aware: the bundle is keyed by this manager's
        resolved Hermes home, so an extension loaded under one profile is
        invisible to every other profile in a multiplexed gateway process.

        Unload is generation-scoped. The returned handle only clears the
        registration if its own generation is still active, so a slow teardown
        of an outgoing generation cannot remove a newer replacement.

        Returns ``None`` if the bundle is invalid; the plugin loads without the
        extension rather than registering a half-valid policy owner.
        ...
        # Keep the outgoing generation observable until the replacement has
        # started successfully. Readers therefore see either the complete old
        # bundle or the complete new bundle, never a half-started generation.
        ...
        # Start before publication. A failed start leaves the complete outgoing
        # generation active and observable.
        ...
        def _release() -> None:
            # Generation-scoped teardown: stop, cancel this generation's tasks,
            # then compare-and-clear. A stale generation's cleanup cannot stop
            # or cancel a newer generation because every step is keyed by this
            # generation id.
            if extension.on_stop is not None:
                try:
                    extension.on_stop(facade)
