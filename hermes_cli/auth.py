...
def _auth_store_lock_for_path(
    ...
    """
    target_path = auth_path or _auth_file_path()
    with _auth_store_lock(timeout_seconds=timeout_seconds, target_path=target_path):
        yield
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    try:
        text = auth_file.read_text(encoding="utf-8-sig")
        raw = json.loads(text)
    except OSError:
