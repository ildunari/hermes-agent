...
def get_profiles_sessions(
    ...
    """Aggregate sessions through database-only profile target discovery."""
    if archived not in ("exclude", "only", "include"):
    ...
    return _read_profile_sessions(
        _profile_session_targets(profile), {"sessions": query}
    )["sessions"]
...
async def search_sessions(
    ...
    try:
        ...
        try:
            ...
            exclude_list = [s.strip() for s in (exclude_sources or "").split(",") if s.strip()]
            excluded_sources = {item.lower() for item in exclude_list}
