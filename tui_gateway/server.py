...
def handle_request(req: dict) -> dict | None:
    ...
    sessions_get = getattr(_sessions, "get", None)
    live_session = sessions_get(sid) if isinstance(sid, str) and callable(sessions_get) else None
    if (
        live_session is not None
        and method not in {"session.close", "terminal.resize", "session.active_list"}
    ):
        refusal = _live_session_browser_annotation_error(live_session, rid)
    ...
    if method == "session.delete" and isinstance(sid, str) and sid:
        ...
        if callable(get_session):
            kind = _browser_annotation_row_kind(db, get_session(sid))
    ...
    with _profile_db(params) as db:
        ...
        try:
            ...
            for s in db.list_sessions_rich(
                ...
                stored_row = get_session(s.get("id")) if callable(get_session) else s
                kind = _browser_annotation_row_kind(db, stored_row or s)
                if kind is None:
        ...
        try:
            ...
            for row in rows:
                ...
                if src in deny:
                    continue
                get_session = getattr(db, "get_session", None)
                stored_row = get_session(row.get("id")) if callable(get_session) else row
                if _browser_annotation_row_kind(db, stored_row or row) is not False:
                    continue
                return _ok(
                    rid,
                    {
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if enabled and (threshold is None or top_up_amount is None):
