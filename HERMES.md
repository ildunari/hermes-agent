...
Do not perform a restart with raw `launchctl`, direct `restart_scope(...)`, inline process killing, `terminal(background=true)`, or `nohup ... &`. In particular, **never use `launchctl submit` for a finite restart or maintenance task**: launchd can infer `KeepAlive`, causing a successful one-shot command to relaunch indefinitely and repeatedly terminate every Hermes surface. Use `launchctl bootstrap` (one-shot) for detached launches, never `launchctl submit`.
