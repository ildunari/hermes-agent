...
Per Kosta's standard flow: each gate is an adversarial diff-based review, dual lanes (Opus 4.8 for architecture/review + GPT-5.6 Terra as second lane; never the implementing model reviewing itself), P0–P3 findings, P0/P1 must be fixed before the phase closes. Reviewers get: the diff file, this plan, read/bash-only tools, high effort, background notification. Every phase ends with `pytest -q` on touched test files + `py_compile` on touched modules + `git diff --check`, evidence pasted into the phase report.
