...
class TestWSL2PowerShellFallback:
    ...
    def test_wsl2_unique_temp_filename(self, monkeypatch, sample_wav):
        ...
        with patch("tools.voice_mode.platform.system", return_value="Linux"), \
             patch("tools.voice_mode._is_wsl2_env", return_value=True), \
             patch("tools.voice_mode._import_audio", side_effect=ImportError), \
             patch("shutil.which", side_effect=lambda x: f"/bin/{x}" if x in ("powershell.exe", "ffmpeg", "ffplay", "sh") else (x if x.startswith("/") else None)), \
