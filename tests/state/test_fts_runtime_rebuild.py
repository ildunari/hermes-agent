...
class TestRuntimeFtsRebuild:
    ...
    def test_operational_error_fts_corruption_reaches_runtime_rebuild(self, db, monkeypatch):
        calls = {"write": 0, "rebuild": []}
        ...
        def _rebuild(exc):
            calls["rebuild"].append(exc)
            return True

        monkeypatch.setattr(db, "_try_runtime_fts_rebuild", _rebuild)
        assert db._execute_write(_flaky) == "ok"
        assert calls["write"] == 2
        assert len(calls["rebuild"]) == 1
        assert isinstance(calls["rebuild"][0], sqlite3.OperationalError)
