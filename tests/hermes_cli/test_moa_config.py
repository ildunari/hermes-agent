...
def test_normalize_moa_config_preserves_named_presets():
    ...
    assert cfg["reference_models"] == [
        {"provider": "openai-codex", "model": "gpt-5.5", "reasoning_effort": "medium", "service_tier": "fast", "enabled": True}
    ]
...
def test_validate_moa_payload_agrees_with_clean_slot():
    """Validation and normalization agree on complete provider/model slots.

    Normalization may enrich a slot with provider defaults, but must not replace
    the provider/model pair accepted at the write boundary.
    """
    from hermes_cli.moa_config import validate_moa_payload
    ...
    aggregator = cfg["presets"]["p"]["aggregator"]
    assert aggregator["provider"] == payload["presets"]["p"]["aggregator"]["provider"]
    assert aggregator["model"] == payload["presets"]["p"]["aggregator"]["model"]
    assert aggregator["reasoning_effort"] == "high"
