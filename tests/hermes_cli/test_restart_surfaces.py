...
def test_late_auto_worker_aborts_when_staleness_cleared(monkeypatch, tmp_path):
    from hermes_cli import restart_surfaces
    ...
    monkeypatch.setattr(restart_surfaces, "LOG_PATH", tmp_path / "restart.log")
    monkeypatch.setattr(restart_surfaces, "targets_for_scope", lambda _scope: (_webui_target(),))
    monkeypatch.setattr(restart_surfaces, "_gateway_busy_details", lambda _targets: [])
    monkeypatch.setattr(restart_surfaces, "_webui_busy_details", lambda _targets: [])
    monkeypatch.setattr(
        restart_surfaces,
        "_webui_restart_condition",
        lambda _condition: (False, "agent_runtime.stale=false"),
    )
    monkeypatch.setattr(
        restart_surfaces,
        "_resolve_loaded_service",
    ...
    monkeypatch.setattr(restart_surfaces, "_kickstart", lambda service: kicked.append(service))
    monkeypatch.setattr(restart_surfaces.time, "sleep", lambda *_args: None)
    ...
    assert restart_scope(
        ...
        abort_if_not="agent_runtime_stale",
    ) == 0
    assert kicked == []
    assert json.loads(marker.read_text())["status"] == "aborted_condition_cleared"


def test_auto_worker_fails_closed_when_staleness_unknown(monkeypatch, tmp_path):
    from hermes_cli import restart_surfaces
    ...
    monkeypatch.setattr(
        restart_surfaces,
        "_webui_restart_condition",
        lambda _condition: (None, "health invalid"),
    )
    monkeypatch.setattr(
        restart_surfaces,
        "_resolve_loaded_service",
    ...
    monkeypatch.setattr(restart_surfaces, "_kickstart", lambda service: kicked.append(service))
    monkeypatch.setattr(restart_surfaces, "_wait_for_scope_health", lambda *_a, **_k: ([], []))
    monkeypatch.setattr(restart_surfaces.time, "sleep", lambda *_args: None)
