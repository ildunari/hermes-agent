...
def test_fallback_persists_providers_block_context_length():
    """Fallback activation must resolve the fallback model's provider override."""
    agent = _make_agent_with_compressor()
    agent._config_context_length = 32_768
    agent._fallback_model = {
        "provider": "vibeproxy",
    ...
    fb_client = MagicMock()
    fb_client.base_url = "http://127.0.0.1:8485/v1"
    fb_client.api_key = "dummy"
    agent._emit_status = lambda msg: None

    with (
        patch("agent.auxiliary_client.resolve_provider_client", return_value=(fb_client, None)),
