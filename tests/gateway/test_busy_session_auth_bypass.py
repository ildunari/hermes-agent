...
def _make_runner(authorized_users=None):
    ...
    runner._route_busy_event_through_extension = lambda event: None
...
# ---------------------------------------------------------------------------
# Generic conversation-extension admission on the busy fast path
# ---------------------------------------------------------------------------


class TestBusySessionExtensionAdmission:
    def test_no_extension_baseline_does_not_mutate_ordinary_event(self):
        from gateway.run import GatewayRunner

        runner = object.__new__(GatewayRunner)
        event = _make_event(user_id="ordinary")
        source = event.source
        ...
        assert routed_source is source
        assert routed_event is event
        assert source.profile is None
        assert source.chat_id_alt is None
        assert source.extension_route_admitted is False

    def test_generic_route_is_applied_before_busy_authorization(
        ...
        runner._extension_profile_is_ready = lambda scope: True
        runner._served_profile_names = lambda: ("default", "isolated")
        runner._permitted_extension_routes = lambda: {"default": ("isolated",)}
        monkeypatch.setattr(run_module, "_load_gateway_config_for_profile", lambda _: {})
        monkeypatch.setattr(
            runtime, "profile_requirements_satisfied", lambda **kwargs: (True, "")
        )
        monkeypatch.setattr(runtime, "build_route_context", lambda *a, **k: object())
        decision = SimpleNamespace(admitted=True)
        monkeypatch.setattr(runtime, "admit_and_route", lambda *a, **k: decision)

        event = _make_event(user_id="external")
        routed_source = dataclasses.replace(
            event.source, profile="isolated", extension_route_admitted=True
        )
        routed_event = dataclasses.replace(event, source=routed_source)
        runner._apply_extension_route_decision_safe = (
            lambda source, original, resolved: (routed_source, routed_event)
        )

        assert runner._route_busy_event_through_extension(event) is routed_event

    @pytest.mark.asyncio
    async def test_busy_path_trusts_only_the_core_stamped_route(self):
        from gateway.run import GatewayRunner

        runner, _ = _make_runner(authorized_users=set())
        runner._is_user_authorized = lambda source: bool(
            source.extension_route_admitted
        )
        routed_source = dataclasses.replace(
            _make_event(user_id="external").source,
            profile="isolated",
            extension_route_admitted=True,
        )
        routed_event = _make_event(user_id="external")
        routed_event.source = routed_source
        runner._route_busy_event_through_extension = lambda event: routed_event
        runner._session_key_for_source = lambda source: "routed-session"
        runner._adapter_for_source = lambda source: None

        assert await GatewayRunner._handle_active_session_busy_message(
            runner, routed_event, "raw-session"
        ) is False

    @pytest.mark.asyncio
    async def test_busy_path_drops_an_extension_denial(self):
        from gateway.run import GatewayRunner

        runner, _ = _make_runner(authorized_users={"user1"})
        runner._route_busy_event_through_extension = lambda event: False
        assert await GatewayRunner._handle_active_session_busy_message(
            runner, _make_event(user_id="user1"), "session"
        ) is True
